...
async def restart_(message: Message):
    """ restart Damien-X """
    await message.edit("`Restarting Damien-X Services`", log=__name__)
    LOG.info("Damien-X Services - Restart initiated")
...
@userge.on_cmd(
    "shutdown", about={"header": "shutdown Damien-X :)"}, allow_channels=False
)
...
async def die_(message: Message) -> None:
    """ set offline timeout to die Damien-X """
    global MAX_IDLE_TIME  # pylint: disable=global-statement
...
@userge.on_cmd(
    "sleep (\\d+)",
    about={"header": "sleep Damien-X :P", "usage": "{tr}sleep [timeout in seconds]"},
    allow_channels=False,
)
